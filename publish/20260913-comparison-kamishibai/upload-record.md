...
| 2026-09-13 | v2 | SkhboCuCiJo | **削除予定**（v3 差し替え後） | 18:24（終了カード込み）。v1 のレビュー反映（二組→フタクミ、S1 時計 3:33、S1/S3/S6/S7/S9 の写真差し替え・前出し）。サムネ **A（暫定。ユーザー選択後に thumbnails().set で差し替え）**、字幕 ja、概要欄 description-v2.txt（章 10、v2 timeline）。 |
| 2026-09-13 | v3 | IsCddwR1n4c | **非公開（試写用）** | 17:58（終了カード込み）。試写 FB「AI 独特の表現」→ 台本 v4（§9 に従い全編書き直し。骨組みの宣言・対句・読点刻み・理論名の先出し・主語のない要約を除去）。readings に銀の方が上／一場面を追加。サムネ **A（暫定）**、字幕 ja、概要欄 description-v2.txt（章 10、v3 timeline）。 |
