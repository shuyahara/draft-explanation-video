"""試写室ページ生成: レンダ出力を3パートに軽量エンコードし、base64 埋め込みの HTML を作る。
  ...
  .venv\\Scripts\\python.exe publish/20260904-dopagaki-kamishibai/make_shisha.py --build dopagaki-kamishibai-v1 --label v1
...
TITLE = "なぜ現代人はドパガキになってしまうのか（めたん・ずんだもん 紙芝居版）"
# パート分割（シーン番号の範囲）と短い章名
PARTS = [
    ("パート1", (1, 4), "夜ふかしの自白〜ドーパミンは快楽物質ではない"),
    ("パート2", (5, 7), "47秒の誤読〜二千年続く不安の系譜"),
    ("パート3", (8, 10), "何かは起きている〜真因と結論"),
]
SCENE_LABELS = {
    1: "フック（夜ふかしの自白 → 命名 → 問い）", 2: "通説の提示（ドーパミン中毒という筋書き）",
    3: "ドーパミンは、快楽の物質なのか（シュルツ）", 4: "欲しい と 好き（ベリッジ）",
    5: "「47秒」の誤読（グロリア・マーク）", 6: "この不安は、いつから続いているのか（文字・読書熱）",
    7: "ワーサムの捏造・学力パニック・モラルパニック", 8: "本当に、何も起きていないのか（反証の検討）",
    9: "わたしたちは、何に負けているのか（真因）", 10: "結論（冒頭に戻る）",
}
...
def main() -> None:
    ...
    for idx, (pname, (s0, s1), pdesc) in enumerate(PARTS):
        ...
        page = f"""<title>ドパガキ紙芝居版 試写室 {pname}</title>
