...
| v2 | BeotcrzVwEk | 非公開（31:11）→ v4 処理完了後に削除 | 2026-09-05 | build/cynicism-kamishibai-v2。台本 v4（冒頭を有名人のボランティア投稿→売名に変更、レビュー一式＝7観点・総合・セリフ×2・つなぎ・間を反映、S4 radiate 図解、S12 章カード追加）。サムネ C、字幕 ja。概要欄 description-v2.txt。既知の軽微な不具合: S4 テロップの「1983–84」のダッシュが□表示（修正は次レンダで） |
| v3 | kERO4v0oSLc | 非公開（31:11）→ 「無力感」の読みが直っていない（辞書 priority 5 で負け）。v4 処理完了後に v2 と共に削除予定 | 2026-09-05 | build/cynicism-kamishibai-v3。S4 テロップのダッシュは修正済み |
| v4 | v-8HlXEvNOE | **非公開・試写用（31:10）** | 2026-09-05 | build/cynicism-kamishibai-v4。v3 ＋ 「無力感→ムリョクカン」（VOICEVOX 辞書 priority 10、tts.py 修正）。ASR で読みを確認済み。概要欄 description-v3.txt（章 8 件）、サムネ C、字幕 ja |
