...
- 台本全文＋観点4つを渡すため出力が長くなりやすい。「usage limit」「at capacity」等の
...
DIALOGUE_HEADER_TEMPLATE = (
    ...
    "## 出力形式\n"
    "次の4セクションを、この順で Markdown で出力してください。\n"
    ...
    "最後に「## 最優先で直すべき5点」として、上記4観点を横断して優先度順に5件挙げて"
    "ください。\n\n"
...
NARRATION_HEADER_TEMPLATE = (
    ...
    "ものです。用語の初出順・冗長さ・反復等の文章レベルのチェックは別のレビューで扱うため、"
    "本レビューでは対象にしないでください。\n\n"
    "## レビュー観点\n"
    f"{OBSERVATION_POINTS}\n\n"
    "## 出力形式\n"
...
def build_output(
    ...
    header = (
        ...
        f"理解度（一文ごと）・説明の抜け・主題への適合・直接表現の4観点でレビュー。"
        f"Codex CLI（GPT）経由。\n\n"
        f"- 日時: {now}\n"
