...
「セリフとして自然か・キャラらしいか・表情タグが合っているか」の観点でレビューさせるツール。
...
## 使い方
    ...
    "C:\\Users\\shuya\\Projects\\script-to-video\\.venv\\Scripts\\python.exe" ^
        tools/review/review_dialogue.py scripts/20260904-dopagaki-kamishibai/20260904-dopagaki-kamishibai.md
...
- セリフ全文＋観点5つ（自然さ・キャラらしさ・テンポ・表情タグ・面白さ）を渡すため出力が
  長くなりやすい。「usage limit」「at capacity」等のクォータ超過で失敗することがあり、
  その場合は60秒待って1回だけ再試行する。
"""
...
PROMPT_HEADER_TEMPLATE = (
    ...
    "## 出力形式\n"
    "Markdown の表で出力してください。列は「シーン／行／話者／種別（1〜5）／該当セリフ"
    "（短く引用）／指摘／言い直し案（表情変更案を含む）」としてください。"
...
def build_output(
    ...
    header = (
        ...
        f"セリフの自然さ・キャラらしさ・掛け合いのテンポ・表情タグの妥当性・面白さの5観点で"
        f"レビュー。Codex CLI（GPT）経由。\n\n"
        f"- 日時: {now}\n"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="対話台本（めたん・ずんだもん）のセリフを GPT（Codex CLI）に自然さ・キャラらしさ・表情タグの観点でレビューさせる"
    )
