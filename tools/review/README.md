...
4. 直接表現（比喩・婉曲・対句の決め文を直接表現に言い直した案。比喩の方が分かりやすいと判断
   した場合はその旨を明記）
5. 冗長・婉曲（表現。2026-09-12 追加）: 4.の決め文以外の一般的な文を対象に、回りくどい言い
   回し・比喩婉曲で言い換えている箇所・同じ内容の二度言い・抽象語（「効く」「向かう」
   「用意する」等）で済ませていて具体語に直せる箇所を、各指摘「元の文 → 簡潔な言い換え案」
   付きで列挙
6. 構成の切れ味（議論。2026-09-12 追加）: 結論（タイトルの問いへの答え）が最初に示唆される
   シーン、章の順序が論証として最短か、無くても結論が変わらない章・段落・研究紹介、
   対話台本では聞き手の相づち・言い直しで議論が足踏みしている箇所を、各指摘
   「削る／前に出す／一文にまとめる」の処方付きで列挙

観点5・6は、「結論がスッと入ってくる」ことを重視する台本の方針（2026-09-12 ユーザー指示）を
受けて追加した。既存の4観点（一文の理解可能性・段落の主題適合）とは異なり、表現の回りくどさと
構成の遠回りを対象にする。
