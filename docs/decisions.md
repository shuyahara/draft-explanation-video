...
## 2026-09-08 ストック写真の制約を撤廃、主要場面は生成イラストへ（男女論ずんだもん版 v5 試写）

- 指摘: 表示画像が全体的に婉曲的（手元・スマホ・人混みばかりで台詞とつながらない）。原因は「顔を写さない・実在 UI や読める文字を写さない」の自主制約。
- 決定: ①主要場面（燃えている投稿、責め合い、反応順の画面、名指しへの二つの反応、書き手の属性など）は Codex で編集イラスト調に生成して直接描く。②ネット上の写真（ストック）については上記制約を外す（人物・団体が特定できる画や権利上の問題は引き続き避ける）。生成画像の「文字は描かない」（偽文字対策）は継続。
- あわせて板・図解の見出しも直接表現に（「怒りが書きやすくなる理由」→「ネットで怒りを投稿しやすい理由」、「感情の言葉」→「怒りの言葉」）。定義だけの板（「ネットの呼び名＝…」）や一文を分断しただけの板は、図解か写真に置き換える。
- 図解: 収束図解の項目が重なる（MECE でない）→ 上位の軸で 2 列の sketch に。並列 4 セルの対比には縦矢印。自明な基準値「1倍」は書かず矢印の先に倍率。研究どうしの数字を比べる台詞（「さっきの二割より」）は書かず、同一研究内の比較だけを一本で言う。
- 一般化先: docs/dialogue-guide.md §6 板の規則 ⑧〜⑫・写真の規則、docs/diagram-guide.md チェックリスト、.claude/commands/draft.md 絵コンテ点検項目、docs/asset-workflow.md 2026-09-08 追記。

## 2026-09-09 淫夢ずんだもん版 試写1回目の指摘を一般化（#32）

- 字幕・板・図解の数字は算用数字にする（年・日付は必須。助数詞つきの「一つ／二つ」は漢数字のまま）。一般化先: CLAUDE.md「機械音声向けの書き方」、docs/dialogue-guide.md §7。
- 研究者は肩書きまで一次資料で確認して本文で言う（教授／准教授／当時博士課程）。日本の大学院生は名前を出さず「〜大学の大学院生が」にする。一般化先: CLAUDE.md「引用・ファクトの扱い」、docs/dialogue-guide.md §7。
- 多読みの語（「表に」等）は readings 登録の候補に入れ、preflight の読み突合で確認する。一般化先: docs/dialogue-guide.md §7。
