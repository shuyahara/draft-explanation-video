"""採用した貼り写真を render-assets-inmu/scene_NN_beat{slot}.* として配置する。

  .venv\Scripts\python.exe scripts/20260909-inmu-kamishibai/stage_assets.py
...
SRC = Path(r"C:\Users\shuya\Projects\assets-kamishibai\photos\candidates-inmu")
DST = Path(r"C:\Users\shuya\Projects\assets-kamishibai\render-assets-inmu")
...
def main() -> None:
    ...
    for ckey in sorted(used, key=lambda k: SOURCE_FILE[k]):
        print(f"  {SOURCE_FILE[ckey]:<52} {ckey:<22} {' '.join(used[ckey])}")
