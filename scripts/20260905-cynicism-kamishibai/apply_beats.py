...
GLOBAL_READINGS = [
    # v7 試写＋audio_query 突合で見つかった誤読（怒り→オコリ、敵同士→カタキドウシ、一段上→イチダンジョウ、他の→タノ、妬み→ソネミ）
    ("怒り", "イカリ"), ("敵同士", "テキドウシ"), ("一段上", "イチダンウエ"), ("他の集団", "ホカノシュウダン"),
    ("同じ人", "オナジヒト"),  # 「同じ人」は未登録だとドウジジン、「同じ」を登録するとオナジジンになる。複合語で登録する
    ("妬み", "ネタミ"),  # 「同じ→オナジ」は登録すると「同じ人」が「オナジジン」になる副作用があり削除（v10 に 1 か所残存） ("妬まれ", "ネタマレ"), ("嫉妬", "シット"),
...
# シーン番号が元台本とずれた（S3 削除）ので、ナレーション版 YAML の readings は番号ではなく
# 「表記が本文に含まれるか」で全シーンに転記する（GLOBAL と同じ扱い）。
_V5 = Path("C:/Users/shuya/Projects/draft-explanation-video/scripts/20260827-cynicism/20260827-cynicism.yaml")
...
GLOBAL_READINGS += [("無力感", "ムリョクカン"), ("甕", "カメ")]
...
BEATS = {
    1: [
        ...
        img("そういう態度のことを、冷笑って呼ぶのよ", 3, "thumb", "【定義】暗がりで光る画面に指を置く手。定義のテロップを重ねる",
            telop="冷笑＝本気を、一段高いところから鼻で笑って済ませる態度"),
    ],
    2: [
        img(None, 2, "office", "【冷笑系】SNS を見る手元。言葉の来歴に入る"),
        img("言葉の由来は、およそ2400年前", 1, "agora", "【古代ギリシャ】アテナイの遺跡へ切り替え"),
        img("なぜ人は、冷笑をしてしまうのか", 3, "agora", "【問いの提示】遺跡の写真を保持し、黒板に問いを重ねる",
            telop="なぜ人は、冷笑してしまうのか"),
    ],
    3: [
        chapter(),
        img(None, 1, "waterhouse", "【犬のような者たち】ウォーターハウス「ディオゲネス」（語源の説明の間だけ）",
            telop="キュオーン（犬）→ キュニコス（犬のような者たち）→ Cynic"),
        img("ところが、19世紀半ばに", 2, "dictionary", "【19世紀の語義】古い辞書のページ（文字は読めない）",
            telop="19世紀半ば: 「人の動機を疑って鼻で笑う性格」"),
        diagram("昔のシニシズムは、自分で実践して", "【対比: 昔と今】実践して変えようとする／実践する人を笑う、を左右 2 列で。決め文でキャプション",
                sketch([[cell("old", "昔のシニシズム", "history_edu", at="昔のシニシズムは、自分で実践して"),
                         cell("old_t", "実践して変えようとする", "directions_run", at="昔のシニシズムは、自分で実践して", after="変えようとした")],
                        [cell("now", "今の冷笑", "smartphone", at="今の冷笑は、自分で実践する人を笑うの"),
                         cell("now_t", "実践する人を笑う", "sentiment_dissatisfied", at="今の冷笑は、自分で実践する人を笑うの", after="笑うの")]],
                       arrows=[("old", "old_t"), ("now", "now_t")],
                       caption={"text": "実践する側から、笑う側へ", "at": "この反転が、今日の話の土台よ"})),
    ],
    4: [
        chapter(),
        ...
        diagram("批判する側も、擁護する側も", "【対比: 敵同士が同じことをする】両側から中央の決めつけへ矢印",
                sketch([[cell("a", "批判する側", "person", at="批判する側も、擁護する側も", after="批判する側"),
                       ...
                       arrows=[("a", "m"), ("b", "m")])),
        img("冷笑が楽な理由の一つ目は", 2, "fundraiser", "【決め文】寄付箱の写真に戻し、一つ目のテロップ", telop="楽な理由①：考えなくて済む"),
    ],
    5: [
        ...
        diagram("同じ質問を、オランダの", "【実験: 674 人の答え】質問 → 多数派の答えと割合",
                sketch([[cell("q", "674人に質問", "groups", at="あわせて674人にね", after="674人"),
                         cell("a", "皮肉屋が賢い", "psychology", at="調べ方によって、56パーセント", after="56パーセント", value="56〜70%")]],
                       arrows=[("q", "a")])),
        img("しかも、その「賢そう」という印象は", 2, "meeting", "【決め文】腕組みの写真に戻し、二つ目のテロップ", telop="楽な理由②：何もしなくても賢い側に立てる"),
    ...
    6: [
        chapter(),
        diagram(None, "【図解: 楽な理由三つ】前章の二つに三つ目が加わり「冷笑は楽」へ収束する（構造が主張）",
                {"type": "narrative", "layout": "converge",
                 "items": [
                     {"id": "think", "text": "考えなくて済む", "at": 1},
                     {"id": "smart", "text": "賢く見える", "at": 1},
                     {"id": "guard", "text": "自分を守れる", "at": 1, "after": "自分を守れること"},
                 ],
                 "result": {"text": "冷笑は楽", "at": 1, "after": "自分を守れること"}}),
        diagram("つまり冷笑は、何かから", "【言葉: 何を守っているのか】盾のピクトグラムに問いを添える",
                sketch([[cell("shield", "何から守っている？", "shield", at="つまり冷笑は、何かから", after="守ろうとしているのね")]],
                       caption={"text": "守っているものは、三つ", "at": "つまり冷笑は、何かから"})),
        img("日本財団が2019年に行った", 1, "election", "【日本の若者】選挙ポスターの前を歩く人々",
            telop="「自分で国や社会を変えられる」日本 18.3%（9カ国中最下位）"),
        diagram("と思っていると、変えようとする人まで", "【言葉: 前提→見え方】どうせ変わらない → 変える人が愚かに見える",
                sketch([[cell("premise", "どうせ変わらない", "sentiment_neutral", at="と思っていると、変えようとする人まで"),
                         cell("view", "変える人が愚かに見える", "visibility", at="と思っていると、変えようとする人まで", after="見えてしまうのよ")]],
                       arrows=[("premise", "view")])),
        ...
        diagram("社会なんて変えられないと思うと", "【対比: 狐と冷笑は同じ形】葡萄→酸っぱいことにする／社会→変えようとする人を嗤う",
                sketch([[cell("g1", "届かない葡萄", "nutrition", at="社会なんて変えられないと思うと"),
                        ...
                        [cell("s1", "変えられない社会", "public", at="社会なんて変えられないと思うと", after="変えられないと思うと"),
                         cell("s2", "変えようとする人を笑う", "sentiment_very_dissatisfied", at="社会なんて変えられないと思うと", after="笑ってしまうことがある")]],
                       arrows=[("g1", "g2"), ("s1", "s2")],
    ...
    7: [
        ...
        diagram("この実験で嫌われたのは", "【対比: 嫌われるのは誰か】優れた人だから、ではなく、自分を責めそうな人",
                sketch([[cell("sup", "優れた人だから？", "workspace_premium", at="この実験で嫌われたのは"),
                         cell("blame", "自分を責めそうな人", "psychology_alt", at="自分を責めてきそうだ、と感じさせる人だったのよ", after="感じさせる人")]],
                       highlight={"ids": ["blame"], "at": "自分を責めてきそうだ、と感じさせる人だったのよ"})),
    ],
    8: [
        ...
        diagram("フェザーが2002年に行った実験では", "【対比: 嫉妬か憤りか】予測したのは憤り。ずんだもんの復唱で憤りが点灯",
                sketch([[cell("envy", "嫉妬", "mood_bad", at="転落を喜ぶ気持ちに強く結びついていたのは", after="嫉妬そのものではなかった"),
                         cell("unfair", "不当だという憤り", "gavel", at="「あの評価は不当だ」という憤りだったわ", after="憤りだったわ")]],
                       caption={"text": "転落を喜ぶ気持ちを予測したのは", "at": "フェザーが2002年に行った実験では"},
                       highlight={"ids": ["unfair"], "at": "不当だ、という気持ち"})),
    ],
    9: [
        img(None, 2, "outside", "【一枚の地図】集団の中の一人（人が他の集団をどう見るか）", telop="Fiske, Cuddy, Glick & Xu (2002) JPSP"),
        diagram("「温かさ」と「有能さ」よ", "【言葉: 二つの軸】温かさ／有能さ のピクトグラム",
                sketch([[cell("warm", "温かさ", "favorite", at="「温かさ」と「有能さ」よ", after="温かさ"),
                         cell("comp", "有能さ", "workspace_premium", at="「温かさ」と「有能さ」よ", after="有能さ")]],
                       caption={"text": "Fiske ら 2002 の二つの軸", "at": "「温かさ」と「有能さ」よ"})),
        ...
        img("一方、本気で何かを変えようとする人は", 1, "beach", "【結果の出ていない挑戦者】水辺でごみを拾う人々（遠景）。冒頭の投稿の中身"),
    ...
    10: [
        ...
        diagram("傷つく前に切り捨てるから", "【代償: 四つ】ここまで一枚ずつ見せた項目を 2×2 で一望にする",
                sketch([[cell("c1", "認知能力", "school", at="傷つく前に切り捨てるから", value="低い"),
                       ...
                       caption={"text": "冷笑の代償", "at": "傷つく前に切り捨てるから"})),
        diagram("精神科医のジョージ・ヴァイラント", "【対比: 成熟した防衛と未成熟な防衛】ユーモア／受動攻撃",
                sketch([[cell("humor", "ユーモア＝成熟", "mood", at="ユーモアは成熟した側"),
                         cell("passive", "受動攻撃＝未成熟", "sentiment_very_dissatisfied", at="一方、直接ぶつからず", after="未成熟な側に置いたわ")]],
                       caption={"text": "「冗談だった」は未成熟の側", "at": "「冗談だった」と逃げられる皮肉は"})),
        board("冷笑は楽", "【決め文1】", telop="冷笑は楽"),
        diagram("でも、楽な分だけ", "【言葉: 楽→代償】決め文をピクトグラムで受ける",
                sketch([[cell("easy", "楽", "sentiment_satisfied", at="でも、楽な分だけ"),
                         cell("cost", "後で代償を払う", "payments", at="でも、楽な分だけ", after="代償を払う")]],
                       arrows=[("easy", "cost")])),
    ],
    11: [
        chapter(),
        img(None, 1, "tv", "【勝ち負けの報道】テレビスタジオ", telop="Cappella & Jamieson (1997) Spiral of Cynicism"),
        img("32の研究、およそ", 4, "tv", "【メタ分析】スタジオの写真を保持し、数字をテロップに",
        ...
        img("日本で目立つのは", 5, "diet", "【決め文】国会の写真を保持し、決め文をテロップに", telop="日本は「不信の国」ではなく「無力感の国」"),
    ...
    13: [
        img(None, 1, "train", "【冒頭へ回帰】電車のスマホ画面を再掲"),
        diagram("信じる力も、つながり直す力も", "【言葉: 削られる三つの力】信じる／つながり直す／動き出す",
                sketch([[cell("trust", "信じる力", "favorite", at="信じる力も、つながり直す力も", after="信じる力"),
                         cell("link", "つながり直す力", "link", at="信じる力も、つながり直す力も", after="つながり直す力"),
                         cell("move", "動き出す力", "directions_run", at="信じる力も、つながり直す力も", after="動き出す力")]])),
        img("もう一回ちゃんと読んでみるのだ", 2, "reply", "【あの投稿を読み直す】返信欄を見る手元（再掲）"),
        board("冷笑は楽", "【決め文】黒板で受ける", telop="冷笑は楽。だから、広がる"),
