...
CREDIT = {
    ...
    "stairs": "Photo: Barbara Olsen / Pexels",
    "lab": "Photo: Tima Miroshnichenko / Pexels",
    "rat": "Photo: Nikolett Emmert / Pexels",
    "office": "Photo: Christina Morillo / Pexels",
    ...
    "classroom": "Photo: YU HSIU CHOU / Pexels",
    "library": "Photo: Pixabay",
    "capitol": "Photo: Guohua Song / Pexels",
    "skinner": "Wikimedia Commons, Public Domain, Luis Dantas",
...
BEATS = {
    1: [
        img(None, 1, "bed", "【夜ふかしの自白】布団の中のスマホの光。冒頭の場面をそのまま見せる"),
        img("そのとき、コメント欄", 2, "hand", "【ドパガキという言葉】コメント欄を見た手元。命名の場面へ",
    ...
    2: [
        ...
        img("ドパガキって言葉も", 3, "stairs", "【デトックス】スマホを離れた場所に置く"),
        board("ただ、この説明には", "【決め文】写真をはずして間。反転の一撃", telop="ドーパミン ≠ 快楽物質"),
    ],
    3: [
        chapter(),
        img(None, 1, "lab", "【1997年の実験】研究室でモニターに向かう研究者", telop="Schultz et al., Science (1997)"),
    ...
    4: [
        ...
        board("ベリッジはこれを", "【二つの言葉】黒板に wanting / liking", telop="wanting（欲しい）／ liking（好き）"),
        img("それ、そのまんま", 2, "bed", "【冒頭の再演】昨夜のずんだもん＝冒頭の写真を再掲"),
    ],
    5: [
        img(None, 1, "office", "【47秒】複数ウィンドウを行き来するオフィスの手元", telop="Gloria Mark（UC Irvine）"),
        board("マーク自身の振り返り", "【数字】黒板に 2分半 → 47秒", telop="2004年: 約2分半 → 近年: 平均47秒（中央値 40秒）"),
        img("好きな映画なら", 2, "movie", "【映画は2時間見られる】映画館"),
        img("面白い本なら", 3, "cafe", "【本は1時間読める】読書の場面"),
        board("少なくとも、この数字だけでは", "【誤読の修正】写真をはずし、出典を残す", telop="Gloria Mark『Attention Span』(2023)"),
    ],
    6: [
        chapter(),
    ...
    7: [
        ...
        img("日本にも、似た騒ぎ", 3, "classroom", "【日本の学力パニック】教室", telop="PISA 読解力 2003年: 8位 → 14位"),
        board("その後、日本の読解力", "【回復】黒板に順位を書き足す", telop="PISA 読解力 2022年: 81か国・地域中 3位"),
    ...
    8: [
        ...
        img("プラットフォームの内側", 2, "capitol", "【内部告発の証言】議会の建物", telop="米上院 商務委員会 公聴会 (2021)"),
        board("ただし、ここが大事", "【ただし】因果の向きは決められない", telop="関連：ある（中程度）／ 因果：決められない"),
        board("もうひとつ、規模", "【規模の数字】0.4%", telop="Orben & Przybylski (2019)：影響は最大 0.4%"),
        board("ここで一度、整理", "【整理】判定ボード3行そろう。決め文まで保持", telop="関連：ある（中程度）／ 因果：決められない ／ 規模：大きくない"),
    ],
    9: [
        chapter(),
        img(None, 1, "skinner", "【スキナーの実験】スキナー箱（装置）", telop="可変比率強化（Ferster & Skinner, 1957）"),
        img("スロットマシンが人を離さない", 2, "slot", "【スロットマシン】カジノ"),
        img("ずんだもんのスマホのフィード", 3, "thumb", "【あなたのスマホ】フィードを親指で払う手元"),
        board("誰にも分からない", "【設計】写真をはずし、出典を黒板に", telop="Clark & Zack, Addictive Behaviors (2023)"),
        img("しかもスマホの画面は", 4, "thumb", "【段差の除去】無限スクロールの手元（再掲）"),
        board("ドパガキという言葉は、その前提", "【決め文】長い決め文をチョークで短く残す", telop="急所を、正確に突く画面"),
    ],
    10: [
        img(None, 1, "bed", "【冒頭に戻る】同じ寝室の写真"),
        board("弱くなったのは", "【最大の決め文】写真をはずして間だけ"),
        img("通知をひとつ切る", 2, "notif", "【対策】通知を切る手元"),
        img("スマホを、寝室の外で", 3, "facedown", "【寝室の外で充電】置かれたスマホ"),
