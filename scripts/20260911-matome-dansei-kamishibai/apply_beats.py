...
BEATS = {
    ...
    # ---------------- S12 近づく役を、誰がやっているか ----------------
    12: [
        img(None, 1, "phone_b", "【自分でやるしかない】夜のスマホの手元に戻り、近づく役の偏りを数字で見る",
            source="Kreager, Cavanagh, Yen & Yu (2014) J Marriage Fam 76(2)"),
        img("その半年で送られた最初のメッセージは", 2, "phone_b", "【4対1】最初のメッセージの件数の偏りを出す",
            telop="最初のメッセージ 男性142,444件・女性34,960件"),
        img("返事をもらえた割合も見てみましょう", 3, "phone_b", "【返信率は逆】送っている数と返る割合が男女で逆であることを出す",
            telop="未返信率 男性発79%・女性発58%"),
        img("そこまでは言えないの", 4, "phone_b", "【読みすぎない】同じ絵のまま、この数字から言えるところまでの限定へ"),
        ...
        img("自分から近づく役、という点が共通している", 7, "phone_b", "【限定と次への問い】手元の絵に戻し、席の移動とメッセージは同じ行動ではない旨と章末の問いへ"),
    ],
