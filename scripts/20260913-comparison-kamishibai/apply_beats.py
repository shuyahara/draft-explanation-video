...
GLOBAL_READINGS: list[tuple[str, str]] = [
    ("銀が上", "ギンガウエ"),
    ("二組", "フタクミ"),  # VOICEVOX は「ニクミ」と誤読（S7「学生を二組に分けて」・S8「ランダムに二組に分けて」）
...
# 2026-09-13 間レビュー（references/20260913-comparison-kamishibai-pauses-review-v3.md）の
# 推奨値を反映。
...
BEATS = {
    ...
    1: [
        ...
        img("仕事の出来みたいに目盛りのないものは", 5, "office3", "【目盛りのない例】同年代の同僚が並ぶオフィス（S3用候補を流用）に替え、目盛りのない測り方の話に入る（体重計だと体型比較に見えるため変更）"),
        img("測り方なのだ", 5, "phone", "【冒頭へ戻る】S1冒頭のスマホの手元に戻り、フックの余韻を保ったまま章末へ"),
    ],
    # ---------------- S2 「比較」とは何か、答えを先に ----------------
    2: [
        ...
        img("他人を見ながら測っているの", 2, "office2", "【保持の分割】同じオフィス写真を保持したまま20秒超を避けるための継続カット"),
        img("壊れているのは二つ", 3, "office2", "【対比の予告】同じオフィス写真を保持したまま、壊れた二つの前提をテロップで出す",
    ...
    3: [
        chapter(),
        board(None, "【板の進行1/4】見出しだけを出す", telop="フェスティンガーの社会的比較（1954）"),
        board("欲求があるの", "【板の進行2/4】①（自分を評価したい欲求）を書き足す",
              telop="フェスティンガーの社会的比較（1954）\n① 自分を評価したい欲求"),
        img("身長や体重なら", 1, "scale", "【測れるものの例】体重計に乗る足元に替え、目盛りがある例を示す"),
        board("目盛りがないとき、人は", "【板の進行3/4】②（目盛りがない→他人で測る）を書き足す",
              telop="フェスティンガーの社会的比較（1954）\n① 自分を評価したい欲求\n② 目盛りがない→他人で測る"),
        board("近い人を選びやすいの", "【板の進行4/4】③（近い人を選ぶ）を書き足して三つ揃える",
              telop="フェスティンガーの社会的比較（1954）\n① 自分を評価したい欲求\n② 目盛りがない→他人で測る\n③ 相手は、近い人を選ぶ"),
        ...
        img("しかもね、比べた結果は", 4, "mri", "【実験装置・前出し】検査装置の写真に替え、脳の活動にも表れるという移行部でMRIを先出しする",
            source="Fliessbach, K. et al. (2007). Science, 318(5854), 1305–1308."),
        img("ドイツのボン大学などの研究チームが", 5, "mri",
    ...
    4: [
        ...
        img("ただし、ここは注意ね", 4, "podium", "【保持の分割】表彰台の写真に戻し、研究の限定を言う間の継続カット"),
        img("テストで80点でも", 5, "classroom", "【S5用写真を流用】教室の学生の写真に替え、身近な例（テストの点数）からシーン末へ"),
    ],
    # ---------------- S5 相手の、何が見えているのか ----------------
    5: [
        chapter(),
        ...
        img("ただし、この研究が調べたのは対面の大学生活よ", 4, "classroom", "【教室へ戻る】研究の限定を言う位置で教室の写真に戻し、シーン末まで保持"),
        img("つまり、この見えなさは", 5, "classroom", "【保持の分割】同じ教室写真を保持したまま20秒超を避けるための継続カット"),
    ],
    ...
    6: [
        ...
        img("ここから先は、研究じゃなくて私の推論ね", 5, "scroll", "【SNSの偏り・前出し】スクロールする手元に替え、SNSの話に入る位置で先出しする（読書写真のままSNSの話をしないように）"),
        img("投稿されるのは、パーティーと昇進よ", 6, "scroll", "【保持の分割】同じスクロール写真を保持したまま継続カット"),
        img("僕が相手を選ぶ前に、相手の方から流れてくるのだ", 7, "scroll", "【保持の分割】同じスクロール写真を保持したまま継続カット（3）"),
        img("それも、一番いい瞬間ばかりがよ", 8, "scroll", "【保持の分割】同じスクロール写真を保持したまま継続カット（4）"),
        img("推論と言ったのは", 9, "scroll", "【保持の分割】同じスクロール写真を保持したまま継続カット（5）"),
        img("相手を選べなくて、相手のいい瞬間しか見えない", 10, "party", "【パーティーへ戻る】まとめの位置でパーティー写真に戻し、章末の問いへ渡す"),
    ],
    # ---------------- S7 眺めるだけで、気分は落ちるのか ----------------
    7: [
        chapter(),
        ...
        img("気分は、使う前と直後、それからその日の夜", 3, "laptop", "【保持の分割】同じノートPCの写真を保持したまま20秒超を避けるための継続カット"),
        img("全部で3回測ったの", 4, "phone", "【時間差】S1冒頭と同じ夜のスマホの手元（顔なし）に替え、夜になって出る変化を示す"
            "（暗い部屋の顔写真は人種の対比に見えるとの指摘のため差し替え）"),
        img("眺めた組だけ", 5, "phone", "【結果】同じ夜のスマホの手元を保持したまま、9%低下／変化なしをテロップで出す",
            telop="眺める組: 夜の気分 約9％低下／動く組: 変化なし"),
        img("そう、直後には出ないの", 6, "phone", "【保持の分割】同じ夜のスマホの手元を保持したまま20秒超を避けるための継続カット"),
        img("1週間、1日5回ずつ", 7, "phone", "【保持の分割】同じ夜のスマホの手元を保持したまま20秒超を避けるための継続カット（2）"),
        img("眺めた後の時間帯ほど", 8, "phone", "【保持の分割】同じ夜のスマホの手元を保持したまま20秒超を避けるための継続カット（3）"),
        diagram(
            "眺めてから気分が下がるまでに",
            "【因果の鎖】眺めるだけ→妬み→夜の気分が下がる、を1本の鎖で見せる。三要素は「眺める、妬む、気分が下がる、の順ね」の"
            "1キューに語同期。動く組の変化なしはcaptionで補い、決め文「下がらなかったのよ」（間）まで保持する",
            {
                "type": "narrative",
                "layout": "chain",
                "items": [
                    {"id": "watch", "text": "眺めるだけ", "icon": "visibility", "at": "眺める、妬む、気分が下がる、の順ね", "after": "眺める"},
                    {"id": "envy", "text": "妬み", "icon": "mood_bad", "at": "眺める、妬む、気分が下がる、の順ね", "after": "妬む"},
                    {"id": "mood_down", "text": "夜の気分↓9%", "icon": "trending_down", "at": "眺める、妬む、気分が下がる、の順ね", "after": "気分が下がる"},
                ],
                "caption": {"text": "動く組は変化なし", "at": "下がらなかったのよ", "after": "下がらなかったのよ"},
            },
        ),
        img("ただし、下がったのは気分で", 7, "laptop", "【落ち着いて限定】明るい部屋のノートPCの写真に戻し、限定条件を言ってシーン末まで"),
        img("もう分かったのだ", 8, "laptop", "【保持の分割】同じノートPCの写真を保持したまま20秒超を避けるための継続カット"),
    ],
    ...
    9: [
        chapter(),
        board(None, "【道しるべ】使い直す前に、壊れた二つの前提を振り返る板を出す（強調なし）",
              telop=GUIDE_BOARD),
        board("一つ目", "【板の進行1/3】使い直しの板へ切り替え、①（見えない部分も数に入れる）を出す",
              telop="比べ方の使い直し\n① 見えない部分も数に入れる"),
        board("二つ目", "【板の進行2/3】②（話しかける）を書き足す",
              telop="比べ方の使い直し\n① 見えない部分も数に入れる\n② 話しかける"),
        img("相手の見える範囲が広がるかもしれないわ", 1, "cafe9", "【話しかける例】カフェで向かい合って話す写真に替え、二つ目の使い直しを示す"),
        board("三つ目。", "【板の進行3/3】③（比べる相手を自分で決める）を書き足して三つ揃える",
              telop="比べ方の使い直し\n① 見えない部分も数に入れる\n② 話しかける\n③ 比べる相手を自分で決める"),
        img("あまり社交的じゃない知人を", 2, "walk2", "【相手を選ぶ例】男女混成の友人グループが並んで歩く写真に替え、三つ目の使い直しの結果をテロップで出す"
            "（恋愛相手選びに見えるとの指摘で素材を差し替え）",
            telop="比べる相手を変えると、『自分の方が劣っている』感覚が消えた",
            source="Deri, S., Davidai, S., & Gilovich, T. (2017). Study 6A. コーネル大学 154人"),
        img("この実験は社交生活の話だから", 3, "walk2", "【保持の分割】同じ写真を保持したまま継続カット"),
        diagram(
            "銀と銅で、何を見たかによって",
            "【S4の再掲】銀と銅の対比図解をまとめて一度に再掲し、何を基準にしたかで見え方が変わることを思い出させる。"
            "決め文「自分で決めるのよ」（間2.5）まで保持する",
            sketch(
                [
        ...
        img("じゃあ、フィードに出てきた順に", 4, "walk2", "【決め文の保持後】男女混成の友人グループの写真に戻し、シーン末まで保持"),
    ],
    # ---------------- S10 結び（冒頭の夜に戻る） ----------------
    10: [
