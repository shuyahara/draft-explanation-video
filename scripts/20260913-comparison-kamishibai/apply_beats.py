...
# 「二組」も「ニクミ」と誤読するため登録。「一場面」は「イチジョオメン」と誤読するため登録。
...
# 2026-09-13 v4: 文言変更（誰と／何を、の言い回しから、選び方／どこを見ているか、に統一）。
GUIDE_BOARD = "\n".join([
    "比較の、おかしくなっている二つ",
    "① 比べる相手の選び方",
    "② 相手のどこを見ているか",
...
BEATS = {
    ...
    1: [
        ...
        img("3時まで眠れなかった", 3, "clock", "【異常事態】3時台を示す時計に替え、無音でも伝わるようテロップを添える",
            telop="3時まで眠れなかった"),
        img("やめられないわね", 4, "clock", "【保持の分割】同じ時計カットを保持したまま20秒超を避けるための継続カット"),
        img("仕事の出来みたいなものって", 5, "office3", "【目盛りのない例】同年代の同僚が並ぶオフィス（S3用候補を流用）に替え、目盛りのない測り方の話に入る"),
        img("測ることなのだ", 5, "phone", "【冒頭へ戻る】S1冒頭のスマホの手元に戻り、フックの余韻を保ったまま章末へ"),
    ],
    # ---------------- S2 比べるとは、何をしていることか ----------------
    2: [
        img(None, 1, "office2", "【開幕の受け】板が出る前にオフィス写真の気配だけで開く（cue1が板より前のため）"),
        board("それがここで言う「比較」ね", "【用語の定義】比較の定義を黒板に固定し、以後の章が参照できるようにする",
              telop="比較\n＝自分がどのくらいか\n他人を見て測ること"),
        img("仕事ができるか", 2, "office2", "【測る対象の例】オフィスで談笑する写真に替え、日常の「できる／できない」判断の例を示す"),
        img("どこを見ているかよ", 2, "office2", "【対比の予告】同じオフィス写真を保持したまま、おかしくなっている二つをテロップで出す",
            telop="おかしくなっているのは ①相手の選び方 ②どこを見ているか"),
        img("来なくていいわ", 3, "office2", "【保持の分割】同じオフィス写真を保持したまま20秒超を避けるための継続カット"),
        board("それがまず違うの", "【板の進行1/3】見出し＋①だけを出す（②③はまだ）",
              telop="比較についての三つの思い込み\n① 比較は悪い癖"),
        board("上を見るから苦しいというのも", "【板の進行2/3】②を書き足す",
              telop="比較についての三つの思い込み\n① 比較は悪い癖\n② 上を見るから苦しい"),
        board("充実して見えるのにも", "【板の進行3/3】③を書き足して三つ揃える",
              telop="比較についての三つの思い込み\n① 比較は悪い癖\n② 上を見るから苦しい\n③ みんな充実して見える"),
    ],
    # ---------------- S3 なぜ、比べずにいられないのか ----------------
    3: [
    ...
    4: [
        ...
        img("順位が上なのに", 2, "podium", "【保持の分割】同じ表彰台の写真を保持したまま20秒超を避けるための継続カット"),
        ...
        img("テストで80点でも", 3, "classroom", "【S5用写真を流用】教室の学生の写真に替え、身近な例（テストの点数）からシーン末へ"),
    ],
    # ---------------- S5 相手の、何が見えているのか ----------------
    5: [
        chapter(),
    ...
    6: [
        ...
        board("もう一つ、おかしくなっているところがあるの", "【道しるべ】冒頭の受けの直後に板を出す（①比べる相手の選び方）",
    ...
    9: [
        chapter(),
        img("昨日の夜をやり直してみましょう", 1, "phone", "【冒頭に戻る】S1と同じ夜のスマホの手元で、昨日の夜をやり直す場面を開く"),
        board("そこでまず、思い出してほしいの", "【板の進行1/3】使い直しの板へ切り替え、①（見えない部分も数に入れる）を出す",
              telop="比べ方の使い直し\n① 見えない部分も数に入れる"),
        board("他人の落ち込みは、実際より17ポイントも少なく見積もられていたでしょう", "【保持の分割】同じ板を保持したまま20秒超を避けるための継続ビート",
              telop="比べ方の使い直し\n① 見えない部分も数に入れる"),
        board("それから、眺めて終わりにしないこと", "【板の進行2/3】②（話しかける）を書き足す",
              telop="比べ方の使い直し\n① 見えない部分も数に入れる\n② 話しかける"),
        img("聞けば、投稿に出ていない事情も", 2, "cafe9", "【話しかける例】カフェで向かい合って話す写真に替え、二つ目の使い直しを示す"),
        board("もう一つ", "【板の進行3/3】③（比べる相手を自分で決める）を書き足して三つ揃える",
              telop="比べ方の使い直し\n① 見えない部分も数に入れる\n② 話しかける\n③ 比べる相手を自分で決める"),
        img("あまり社交的じゃない知人を", 3, "walk2", "【相手を選ぶ例】男女混成の友人グループが並んで歩く写真に替え、三つ目の使い直しの結果をテロップで出す",
            telop="比べる相手を変えると、『自分の方が劣っている』感覚が消えた",
            source="Deri, S., Davidai, S., & Gilovich, T. (2017). Study 6A. コーネル大学 154人"),
        img("それどころか、生活の満足度は上がったわ", 4, "walk2", "【保持の分割】同じ友人グループの写真を保持したまま20秒超を避けるための継続カット"),
        diagram(
            "銀と銅で、何を見たかで",
            "【S4の再掲】銀と銅の対比図解をまとめて一度に再掲し、何を基準にしたかで見え方が変わることを思い出させる。"
            "決め文「自分で選ぶのよ」（間2.5）まで保持する",
            sketch(
                [
                    [cell("gold2", text="金", at="銀と銅で、何を見たかで", after="銀と銅で、何を見たかで"), None],
                    [
                        cell("silver2", text="銀メダリスト", value="4.8", at="銀と銅で、何を見たかで", after="銀と銅で、何を見たかで"),
                        cell("bronze2", text="銅メダリスト", value="7.1", at="銀と銅で、何を見たかで", after="銀と銅で、何を見たかで"),
                    ],
                    [None, cell("fourth2", text="4位", at="銀と銅で、何を見たかで", after="銀と銅で、何を見たかで")],
                ],
    ...
    # ---------------- S10 結び（もう一通） ----------------
    10: [
        img("続きをやりましょうか", 1, "phone_morning", "【朝に戻る】S1と同じスマホの手元だが朝の光で開く"),
        img("本人の話を聞きに行くの", 2, "coffee", "【裏を見に行く】二人でコーヒーを飲む写真に替え、本人の話を聞きに行く様子を示す"),
        img("じゃあ今夜は、投稿を見返す前に", 3, "coffee", "【保持の分割】同じコーヒーの写真を保持したまま20秒超を避けるための継続カット"),
