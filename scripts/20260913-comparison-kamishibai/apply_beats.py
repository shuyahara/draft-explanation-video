...
BEATS = {
    ...
    5: [
        board(None, "【道しるべ】章頭で板を出す（v11: 1 秒だけ出る写真をやめた）（①比べる相手の選び方）",
            ...
            sketch(
                [
                    ...
                    [
                        ...
                        cell("pa_r", icon="trending_down", text="孤独感が減少", at="こっちは孤独感が減ったわ", after="こっちは"),
                    ],
                ],
                arrows=[("fb_n", "fb_op"), ("fb_op", "fb_r"), ("pa_n", "pa_op"), ("pa_op", "pa_r")],
                highlight={"ids": ["fb_r", "pa_r"], "at": "こっちは孤独感が減ったわ", "after": "こっちは"},
                caption={"text": "8割が『止めてよかった』", "at": "8割の人が", "after": "8割の人が"},
            ),
        ),
        img("やっぱりやめれば効くのだ", 3, "phonedesk", "【引用文のあと】学生の言葉まで図解を保持してから机のスマホに戻す（v11: 結果のセルが出た直後に切り替わっていた）"),
        img("ちょっと待って", 4, "phonedesk", "【決め文まで保持】板にせず机のスマホを保持したまま、決め文「止まらないのよ」（間）を聞かせる"
            "（v10: v9 の板は字幕の写しで、①②が譲歩と反転の関係を列挙に見せていた）"),
        img("SNSを減らすのも効くけど", 5, "phonedesk", "【保持の分割】同じ机のスマホを保持したまま、決め文「止まらないのよ」（間）まで（20秒超を避ける継続カット）"),
    ...
    8: [
        ...
        board("資格の合否、走ったタイム", "【用語と列挙】「目盛りのあるもの」を見出しに、音声では覚えきれない具体例 3 つを板に並べる"
              "（v10: v9 の「比べ方の使い直し／① 目盛りのあるものを持つ」は枠組みラベルの見出しと一項目の番号付きで、板が抽象・台詞が具体と逆だった）",
              telop="目盛りのあるもの\n① 資格の合否\n② 走ったタイム\n③ 書き上げた枚数"),
        img("そこでは他人を見る必要がないから", 3, "timer", "【保持の分割】同じタイマー写真を保持したまま20秒超を避けるための継続カット"),
        img("それから、SNSは眺めるだけで終えないこと", 3, "phone", "【夜のスマホへ】二つ目の提案で S1 と同じ夜のスマホの手元に替え、気まずさの話に入る（v10: 主張文の板をやめた）"),
            ...
            sketch(
                [
                    [
                        ...
                        cell("liu_real", icon="sentiment_very_satisfied", value="6.17点", text="相手の実際の喜び", at="送った側は", after="受け取った側は"),
                    ],
                ],
                arrows=[("liu_pred", "liu_real")],
                highlight={"ids": ["liu_real"], "at": "送った側は", "after": "受け取った側は"},
