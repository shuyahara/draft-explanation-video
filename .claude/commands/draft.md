...
> **対象はナレーション版（社会を科学する雑学）**。めたん×ずんだもんの紙芝居（ずんだ人文学）は
> `/draft-kamishibai` を使う（VOICEVOX・対話の書き方・板・終了カード・タイトル接尾語・サムネの規則が違う）。
...
0. **企画確定（`grilling` スキル併用）**: 執筆に入る前に、`grilling`（Skill ツール）で
   ...
   確定済みの項目は聞き直さない。
0.5. **構成メモで合意を取る（ユーザーの確認はここが最後）**: 手順0の確定事項をもとに、本文を書く前に
   1 ページの構成メモを提示して OK をもらう。含めるもの: ①冒頭の具体例（後半の根拠・実験と**同じ型**
   か） ②章立てと各章の問い ③各章の決め文（**直接表現**で。比喩・婉曲は使わない） ④削る候補
   （主題に寄与しない逸話・雑学） ⑤タイトル案 ⑥図解のイメージ一覧（見せたい図のラフを言葉で）
   ⑦サムネの狙い。書式は `templates/kousei-memo-template.md`（**先頭に 800〜1,200 字の「要約版の本文」**を置き、
   ユーザーが流し読みで流れを確認できるようにする。2026-09-13 指示。点検表は docs/structure-references.md §4）。
   ユーザー指示（2026-09-06）: 方向性の確認はここで済ませ、以降は自分の点検で
   仕上げ品質まで自走し、試写に出すのは一度で済む状態にする。
...
## 仕上げまでの自走（手順 9 のあと。ユーザーの確認を挟まない）

構成メモ（手順 0.5）で合意した後は、次を**すべて自分で**通してから、初めてユーザーに試写を出す
（2026-09-06 ユーザー指示。冷笑ずんだもん版で試写 10 版を要した反省）。

1. 台本レビュー一式（`/review`）: 7 観点 → 総合 → **clarity（理解度・説明の抜け・主題適合・直接表現）**
   → 対話台本ならセリフ・つなぎ → 間。採否を決めて反映し、自己点検（docs/dialogue-guide.md §4b・§5）を通す。
2. YAML 生成 → `validate` → **`tools/preflight.py`**（読みの突合・50 字超キュー・10 秒超の板・描けない文字・
   ラベル長・写真の有無）。該当ゼロになるまで直す。
3. **`storyboard`**（script-to-video）で全ビートの静止画を出し、自分で見る: 写真の選択、図解の大きさ、
   テロップ、板だけの区間。（紙芝居の追加点検は `/draft-kamishibai` 手順 4。以下は共通項目）:
   写真がその台詞の場面・物・人を指しているか（「別カットに替えて」だけの同カテゴリ使い回しは 2 枚まで）／
   用語の定義・主題の定義・要点は「言葉＝説明文」の板になっているか／板の見出しがメタ語（覚えておくこと・まとめ）に
   なっていないか／2 要素以上は ①② の番号か／順序・因果・対比（→ が 2 つ以上）は板でなく図解か／
   板とラベルの語は台詞の具体語か（抽象語・言い換え語なら婉曲）／「A＝B」だけの板・一文を 2 行に割った板がないか／
   格子の図解に矢印か列見出しがあるか、収束図解の項目が重なっていないか／写真は台詞の場面を直接描いているか
   （連想 1 段の写真は無関係に見える。描けない場面は Codex でイラスト生成）（2026-09-08 男女論版 v5 試写から）／
   **数字は算用数字か**（年・日付は必須。助数詞つきの「一つ／二つ」は漢数字のまま）／
   **研究者の肩書きがあるか（教授／准教授／当時博士課程。日本の大学院生は無名のまま）**
   （2026-09-09 淫夢版 試写1回目から。docs/dialogue-guide.md §7）。
4. レンダ → `review_video.py` ＋ `review_reading.py` → 自分でも 20〜30 秒おきのフレーム抜き出しを見る →
   直すべきものは直して再レンダ（自分の判断で 2 回まで）。
5. 概要欄・章時刻・サムネ候補（Codex 生成を含む 3 案）・非公開アップロードまで済ませ、
   「試写用 URL＋変えた点＋ユーザーに決めてほしい点」だけを報告する。
