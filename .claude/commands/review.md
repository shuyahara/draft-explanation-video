...
（`tools/review/review_holistic.py`。面白さ・視聴維持・見やすさ・タイトル）→**③視聴者理解・
主題適合レビュー**（`tools/review/review_clarity.py`。理解度・説明の抜け・主題への適合・
直接表現。対話台本・ナレーション台本どちらも対応）に加え、`--video` 指定時のみ実行する
**④映像レビュー**（`tools/review/review_video.py`）。対象が**対話台本（めたん×ずんだもん
紙芝居版）**の場合は、これに加えて対話専用のレビュー一式（手順6）も回す。
...
1. **対象の特定**: 引数が `scripts/` 配下のフォルダなら、その中の台本 md（フォルダと同名の
   ...
   あるかを確認し、あれば**対話台本（めたん×ずんだもん紙芝居版）**と判別する（手順6の対象）。
...
4. **視聴者理解・主題適合レビュー**（理解度・説明の抜け・主題への適合・直接表現）: 総合
   レビュー（手順3）の指摘を反映した後の台本に対して実行するのが望ましい。対話台本・
   ナレーション台本のどちらにも使える。
   ```
   PYTHONUTF8=1 "C:/Users/shuya/Projects/script-to-video/.venv/Scripts/python.exe" ^
       tools/review/review_clarity.py <台本mdパス>
   ```
   出力先（`references/{実行日}-{台本フォルダ名}-clarity-review.md`）を読む。
5. **映像レビュー**（`--video <レンダ出力dir>` が指定されたときのみ）: 台本と同じフォルダにある
   シーン YAML を対象に実行する。
   ```
   PYTHONUTF8=1 "C:/Users/shuya/Projects/script-to-video/.venv/Scripts/python.exe" ^
...
6. **対話台本専用レビュー**（手順1で対話台本と判別した場合のみ）: `review_dialogue.py`（セリフ）
...
8. **反映**: 採用した指摘を、台本フォーマット規約（CLAUDE.md「台本フォーマット」「引用・ファクトの
...
9. **再検証**: 反映後、YAML を `validate` サブコマンドで再検証する。
   ```
...
- `review_script.py` / `review_holistic.py` / `review_clarity.py` / `review_video.py` /
  `review_dialogue.py` / `review_transitions.py` / `review_pauses.py` は**指摘を生成するだけ**で、
  台本・YAML の文言は一切編集しない。編集は本コマンドの手順8でオーケストレーターが行う。
